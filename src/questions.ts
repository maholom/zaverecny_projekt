type TAnswer = { text: string; value: boolean };

type TQuestion = {
  text: string;
  answers: TAnswer[];
};

export const questions: TQuestion[] = [
  {
    text: "Když potkáš na sjezdovce kamaráda, jak ho pozdravíš?",
    answers: [
      {
        text: "Tschüss",
        value: false,
      },
      {
        text: "Hallo",
        value: true,
      },
      {
        text: "Guten Tag",
        value: false,
      },
    ],
  },

  {
    text: "Na čem si pochutnáš, když si objednáš Kaiserschmarrn?",
    answers: [
      {
        text: "Na sladkém trhanci",
        value: true,
      },
      {
        text: "Na hovězím vývaru",
        value: false,
      },
      {
        text: "Na císařské roštěné",
        value: false,
      },
    ],
  },

  {
    text: "Kde se naučíš lyžovat? Jak se řekne lyžařská škola?",
    answers: [
      {
        text: "Schneeschule",
        value: false,
      },
      {
        text: "Skischule",
        value: true,
      },
      {
        text: "Fahrschule",
        value: false,
      },
    ],
  },

  {
    text: "Co bys našel, když bys v horách hledal Loipe?",
    answers: [
      {
        text: "Perníkovou chaloupku",
        value: false,
      },
      {
        text: "Běžeckou stopu",
        value: true,
      },
      {
        text: "Vlek",
        value: false,
      },
    ],
  },

  {
    text: "Jak se rozloučíš s kamarádem?",
    answers: [
      {
        text: "Tschüss",
        value: true,
      },
      {
        text: "Auf Wiedersehen",
        value: false,
      },
      {
        text: "Hallo",
        value: false,
      },
    ],
  },

  {
    text: "Co si obuješ, když chceš jít bruslit?",
    answers: [
      {
        text: "Lederhose",
        value: false,
      },
      {
        text: "Schlittschuhe",
        value: true,
      },
      {
        text: "Schneeschuhe",
        value: false,
      },
    ],
  },

  {
    text: "Kdo sídlí v budově s nápisem Bergwacht?",
    answers: [
      {
        text: "Rozhlas",
        value: false,
      },
      {
        text: "Prodejna ponožek",
        value: false,
      },
      {
        text: "Horská služba",
        value: true,
      },
    ],
  },

  {
    text: "Co máš v plánu, když jdeš do hor wandern?",
    answers: [
      {
        text: "Jdu na túru",
        value: true,
      },
      {
        text: "Jdu venčit psa.",
        value: false,
      },
      {
        text: "Jdu slavit narozeniny.",
        value: false,
      },
    ],
  },

  {
    text: "Jak pozdravíš v Bavorsku v obchodě?",
    answers: [
      {
        text: "Tschüss",
        value: false,
      },
      {
        text: "Grüß Gott",
        value: true,
      },
      {
        text: "Ahoj",
        value: false,
      },
    ],
  },

  {
    text: "Co si nasadíš na hlavu, abys neměl studené uši?",
    answers: [
      {
        text: "Jacke",
        value: false,
      },
      {
        text: "Hose",
        value: false,
      },
      {
        text: "Mütze",
        value: true,
      },
    ],
  },

  {
    text: "Co si obuješ, aby Ti nebyla zima? Jak se řeknou německy sněhule?",
    answers: [
      {
        text: "Schneestiefel",
        value: true,
      },
      {
        text: "Socken",
        value: false,
      },
      {
        text: "Strumpfhose",
        value: false,
      },
    ],
  },

  {
    text:
      "Když si v horské chatě objednáš Käsespätzle, co v nich určitě najdeš?",
    answers: [
      {
        text: "Povidla",
        value: false,
      },
      {
        text: "Sýr",
        value: true,
      },
      {
        text: "Třešně",
        value: false,
      },
    ],
  },

  {
    text:
      "Mrzneš, ale vystoupal jsi na vrchol hory. Kde se posilníš? Jak se řekne horská chata?",
    answers: [
      {
        text: "Berghütte",
        value: true,
      },
      {
        text: "Plattenbau",
        value: false,
      },
      {
        text: "Villa",
        value: false,
      },
    ],
  },

  {
    text: "V závodě máš pořadové číslo šest. Kolik to je v němčině?",
    answers: [
      {
        text: "Zwei",
        value: false,
      },
      {
        text: "Drei",
        value: false,
      },
      {
        text: "Sechs",
        value: true,
      },
    ],
  },

  {
    text: "Jedeš štafetu. Tvůj tým má modrou barvu. Jak se řekne v němčině?",
    answers: [
      {
        text: "Blau",
        value: true,
      },
      {
        text: "Gelb",
        value: false,
      },
      {
        text: "Rot",
        value: false,
      },
    ],
  },

  {
    text: "V jakém ročním období nejčastěji lyžujeme?",
    answers: [
      {
        text: "Winter",
        value: true,
      },
      {
        text: "Frühling",
        value: false,
      },
      {
        text: "Sommer",
        value: false,
      },
    ],
  },

  {
    text: "Jakým dopravním prostředkem vyjede lyžař nahoru na kopec?",
    answers: [
      {
        text: "Fahrrad",
        value: false,
      },
      {
        text: "Auto",
        value: false,
      },
      {
        text: "Seilbahn",
        value: true,
      },
    ],
  },

  {
    text: "Co si vyzuješ, než jdeš lyžovat?",
    answers: [
      {
        text: "Handschuhe",
        value: false,
      },
      {
        text: "Hausschuhe",
        value: true,
      },
      {
        text: "Mütze",
        value: false,
      },
    ],
  },

  {
    text: "Jakou barvu má sníh?",
    answers: [
      {
        text: "Dunkelbraun",
        value: false,
      },
      {
        text: "Weiß",
        value: true,
      },
      {
        text: "Lila",
        value: false,
      },
    ],
  },

  {
    text: "Jaký je led?",
    answers: [
      {
        text: "glatt",
        value: true,
      },
      {
        text: "warm",
        value: false,
      },
      {
        text: "rot",
        value: false,
      },
    ],
  },

  {
    text: "Kdo ti pomůže, když tě zavalí lavina?",
    answers: [
      {
        text: "Grinch",
        value: false,
      },
      {
        text: "Bergrettungsdienst",
        value: true,
      },
      {
        text: "Weihnachtsmann",
        value: false,
      },
    ],
  },

  {
    text: "Z čeho se skáče na lyžích?",
    answers: [
      {
        text: "Brücke",
        value: false,
      },
      {
        text: "Dach",
        value: false,
      },
      {
        text: "Sprungschanze",
        value: true,
      },
    ],
  },

  {
    text: "Čím si namažeš lyže?",
    answers: [
      {
        text: "Butter",
        value: false,
      },
      {
        text: "Skiwachs",
        value: true,
      },
      {
        text: "Marmelade",
        value: false,
      },
    ],
  },

  {
    text: "Co padá v zimě z nebe?",
    answers: [
      {
        text: "Schnee",
        value: true,
      },
      {
        text: "Eiscreme",
        value: false,
      },
      {
        text: "Frösche",
        value: false,
      },
    ],
  },

  {
    text: "Jaké zvíře můžeš potkat v Alpách?",
    answers: [
      {
        text: "Walfisch",
        value: false,
      },
      {
        text: "Elefant",
        value: false,
      },
      {
        text: "Gämse",
        value: true,
      },
    ],
  },

  {
    text: "Co si oblékneš v zimě na ruce?",
    answers: [
      {
        text: "Handschuhe",
        value: true,
      },
      {
        text: "Socken",
        value: false,
      },
      {
        text: "Badehose",
        value: false,
      },
    ],
  },

  {
    text: "Co má na krku sněhulák z obrázku?",
    answers: [
      {
        text: "Blauer Rock",
        value: false,
      },
      {
        text: "Roter Schal",
        value: true,
      },
      {
        text: "Rosa Brille",
        value: false,
      },
    ],
  },

  {
    text: "Co si nevezmeš na hlavu, když jdeš lyžovat?",
    answers: [
      {
        text: "Topf",
        value: true,
      },
      {
        text: "Skihelm",
        value: false,
      },
      {
        text: "Skimaske",
        value: false,
      },
    ],
  },

  {
    text: "Jaké zvíře nemůžeš potkat v Alpách?",
    answers: [
      {
        text: "Kuh",
        value: false,
      },
      {
        text: "Ziege",
        value: false,
      },
      {
        text: "Löwe",
        value: true,
      },
    ],
  },

  {
    text: "Co se nedá dělat na lyžích?",
    answers: [
      {
        text: "Laufen",
        value: false,
      },
      {
        text: "Schlafen",
        value: true,
      },
      {
        text: "Fahren",
        value: false,
      },
    ],
  },

  {
    text: "Jaké zvíře žije v Alpách?",
    answers: [
      {
        text: "Zebra",
        value: false,
      },
      {
        text: "Strauß",
        value: false,
      },
      {
        text: "Bär",
        value: true,
      },
    ],
  },

  {
    text: "Která rostlina se v Alpách nesmí trhat?",
    answers: [
      {
        text: "Edelweiß",
        value: true,
      },
      {
        text: "Löwenzahn",
        value: false,
      },
      {
        text: "Mohnblume",
        value: false,
      },
    ],
  },

  {
    text: "Které ovoce nemůžeš sbírat na horské tůře?",
    answers: [
      {
        text: "Blaubeeren",
        value: false,
      },
      {
        text: "Bananen",
        value: true,
      },
      {
        text: "Erdbeeren",
        value: false,
      },
    ],
  },

  {
    text: "Jakou barvu mají stromy na obrázku?",
    answers: [
      {
        text: "Grau",
        value: false,
      },
      {
        text: "Gelb",
        value: false,
      },
      {
        text: "Grün",
        value: true,
      },
    ],
  },

  {
    text: "Jaký kroj nosí ženy v Bavorsku?",
    answers: [
      {
        text: "Dirndl",
        value: true,
      },
      {
        text: "Pyjama",
        value: false,
      },
      {
        text: "Overall",
        value: false,
      },
    ],
  },

  {
    text: "Jaké sis vylosoval startovní pořadí, když uslyšíš číslo ACHT?",
    answers: [
      {
        text: "6",
        value: false,
      },
      {
        text: "7",
        value: false,
      },
      {
        text: "8",
        value: true,
      },
    ],
  },

  {
    text: "Co znamená, když na tebe někdo na sjezdovce zakřičí ACHTUNG?",
    answers: [
      {
        text: "Pozor!",
        value: true,
      },
      {
        text: "Dáš si limonádu?",
        value: false,
      },
      {
        text: "Svítí sluníčko.",
        value: false,
      },
    ],
  },

  {
    text: "Co to je jódlování?",
    answers: [
      {
        text: "Skákání do výšky",
        value: false,
      },
      {
        text: "Potápění se",
        value: false,
      },
      {
        text: "Tradiční zpěv alpských horalů",
        value: true,
      },
    ],
  },

  {
    text:
      "Za chvilku musíš jít z bruslení domů. Jak se zeptáš, kolik je hodin?",
    answers: [
      {
        text: "Wie spät ist es bitte?",
        value: true,
      },
      {
        text: "Wie alt bist du?",
        value: false,
      },
      {
        text: "Woher kommst du?",
        value: false,
      },
    ],
  },

  {
    text: "Jak se jmenuje toto písmenko: ß?",
    answers: [
      {
        text: "ostré B",
        value: false,
      },
      {
        text: "ostré A",
        value: false,
      },
      {
        text: "ostré S",
        value: true,
      },
    ],
  },

  {
    text:
      "Po dlouhém dni ve sněhu se jdeš navečeřet. Jak se řekne v němčině večeře?",
    answers: [
      {
        text: "Mittagessen",
        value: false,
      },
      {
        text: "Kleines Essen",
        value: false,
      },
      {
        text: "Abendessen",
        value: true,
      },
    ],
  },

  {
    text: "S čím se podává Apfelstrudel?",
    answers: [
      {
        text: "se salámem",
        value: false,
      },
      {
        text: "s lékořicí",
        value: false,
      },
      {
        text: "s vanilkovou omáčkou",
        value: true,
      },
    ],
  },

  {
    text: "Jak se řekne v němčine Člověče, nezlob se?",
    answers: [
      {
        text: "Mensch ärgere Dich nicht",
        value: true,
      },
      {
        text: "Los geht’s!",
        value: false,
      },
      {
        text: "Du bist dran!",
        value: false,
      },
    ],
  },

  {
    text: "Jak bys odstartoval závod: tři dva jedna akce!?",
    answers: [
      {
        text: "zehn elf zwölf!",
        value: false,
      },
      {
        text: "sechs sieben acht!",
        value: false,
      },
      {
        text: "drei zwei eins los!",
        value: true,
      },
    ],
  },

  {
    text: "Z Mnichova jsi v Alpách co by dup. Jak se Mnichov řekne německy?",
    answers: [
      {
        text: "Minchen",
        value: false,
      },
      {
        text: "Mnichov",
        value: false,
      },
      {
        text: "München",
        value: true,
      },
    ],
  },

  {
    text: "Kdy řekneš v Bavorsku Servus?",
    answers: [
      {
        text: "Když se přivítáš a loučíš.",
        value: true,
      },
      {
        text: "Když někomu šlápneš na nohu.",
        value: false,
      },
      {
        text: "Když rozbiješ okno.",
        value: false,
      },
    ],
  },

  {
    text: "Co se ve Švýcarsku nejvíce podobá našim bramborákům?",
    answers: [
      {
        text: "Rösti",
        value: true,
      },
      {
        text: "Apfelmus",
        value: false,
      },
      {
        text: "Powidl",
        value: false,
      },
    ],
  },

  {
    text: "V horách jsi potkal medvěda. Jak zavoláš o pomoc?",
    answers: [
      {
        text: "Es freut mich!",
        value: false,
      },
      {
        text: "Guten Tag!",
        value: false,
      },
      {
        text: "Hilfe!",
        value: true,
      },
    ],
  },

  {
    text: "Instruktor lyžování se tě zeptal - Wie heißt du? - co to znamená?",
    answers: [
      {
        text: "Kolik ti je let?",
        value: false,
      },
      {
        text: "Kde máš lyže?",
        value: false,
      },
      {
        text: "Jak se jmenuješ?",
        value: true,
      },
    ],
  },

  {
    text: "Jsi lyžař začátečník, takže jsi: ?",
    answers: [
      {
        text: "Anfänger",
        value: true,
      },
      {
        text: "Profi",
        value: false,
      },
      {
        text: "Fortgeschrittener",
        value: false,
      },
    ],
  },

  {
    text:
      "Na obchodě se suvenýry visí cedule s nápisem Geschlossen - co to pro tebe znamená: ?",
    answers: [
      {
        text: "Je zavřeno.",
        value: true,
      },
      {
        text: "Je otevřeno.",
        value: false,
      },
      {
        text: "Dnes prodávají gumové medvídky.",
        value: false,
      },
    ],
  },

  {
    text: "Co získáváme od alpských kraviček?",
    answers: [
      {
        text: "Mehl",
        value: false,
      },
      {
        text: "Milch",
        value: true,
      },
      {
        text: "Schokolade",
        value: false,
      },
    ],
  },

  {
    text: "Ve Švýcarsku se z mléka vyrábí: ?",
    answers: [
      {
        text: "Fleisch",
        value: false,
      },
      {
        text: "Bier",
        value: false,
      },
      {
        text: "Käse",
        value: true,
      },
    ],
  },

  {
    text: "Jakou barvu nenajdeme na německé vlajce?",
    answers: [
      {
        text: "Blau",
        value: true,
      },
      {
        text: "Schwarz",
        value: false,
      },
      {
        text: "Gold",
        value: false,
      },
    ],
  },

  {
    text: "Jaký znak je na švýcarské vlajce?",
    answers: [
      {
        text: "Kreis",
        value: false,
      },
      {
        text: "Kreuz",
        value: true,
      },
      {
        text: "Dreieck",
        value: false,
      },
    ],
  },

  {
    text: "Jak nepopřeju německému kamarádovi dobrou chuť?",
    answers: [
      {
        text: "Guten Appetit!",
        value: false,
      },
      {
        text: "Mahlzeit!",
        value: false,
      },
      {
        text: "Gute Nacht!",
        value: true,
      },
    ],
  },

  {
    text: "Jak se omluvíš, když se s někým srazíš na sjezdovce?",
    answers: [
      {
        text: "Es tut mir Leid.",
        value: true,
      },
      {
        text: "Macht nichts.",
        value: false,
      },
      {
        text: "Nichts zu danken.",
        value: false,
      },
    ],
  },

  {
    text: "Co se těží v Alpách?",
    answers: [
      {
        text: "Zucker",
        value: false,
      },
      {
        text: "Salz",
        value: true,
      },
      {
        text: "Pfeffer",
        value: false,
      },
    ],
  },

  {
    text: "Ve kterém rakouském městě se konaly 2x zimní olympijské hry?",
    answers: [
      {
        text: "Wien",
        value: false,
      },
      {
        text: "Linz",
        value: false,
      },
      {
        text: "Innsbruck",
        value: true,
      },
    ],
  },

  {
    text: "Na čem si pochutnám, když dostanu Gugelhupf?",
    answers: [
      {
        text: "Na bábovce",
        value: true,
      },
      {
        text: "Na bramboráku",
        value: false,
      },
      {
        text: "Na švestce",
        value: false,
      },
    ],
  },

  {
    text: "Jak se jmenuje místo, kde jezdíme na sjezdových lyžích?",
    answers: [
      {
        text: "Straße",
        value: false,
      },
      {
        text: "Piste",
        value: true,
      },
      {
        text: "Schlange",
        value: false,
      },
    ],
  },

  {
    text: "Na co čekáme, když čekáme na Skilift?",
    answers: [
      {
        text: "Na limonádu",
        value: false,
      },
      {
        text: "Na pokladnu",
        value: false,
      },
      {
        text: "Na vlek",
        value: true,
      },
    ],
  },

  {
    text: "Z jakého kovu je vyrobena medaile pro vítěze?",
    answers: [
      {
        text: "Gold",
        value: true,
      },
      {
        text: "Bronze",
        value: false,
      },
      {
        text: "Eisen",
        value: false,
      },
    ],
  },

  {
    text: "Co dělám, když se věnuji Bergsteigen?",
    answers: [
      {
        text: "Posilování",
        value: false,
      },
      {
        text: "Horolezectví",
        value: true,
      },
      {
        text: "Veslování",
        value: false,
      },
    ],
  },

  {
    text: "Co děláme  v zimě na saních?",
    answers: [
      {
        text: "Fliegen",
        value: false,
      },
      {
        text: "Rasieren",
        value: false,
      },
      {
        text: "Rodeln",
        value: true,
      },
    ],
  },

  {
    text: "Co můžu, když je velká zima a málo se obléknu?",
    answers: [
      {
        text: "frieren",
        value: true,
      },
      {
        text: "brennen",
        value: false,
      },
      {
        text: "bremsen",
        value: false,
      },
    ],
  },

  {
    text: "Na čem se nedá lyžovat?",
    answers: [
      {
        text: "Stuhl",
        value: true,
      },
      {
        text: "Snowboard",
        value: false,
      },
      {
        text: "Ski",
        value: false,
      },
    ],
  },

  {
    text: "Kde si můžu půjčit lyže?",
    answers: [
      {
        text: "Imbiss",
        value: false,
      },
      {
        text: "Skiverleih",
        value: true,
      },
      {
        text: "Haltestelle",
        value: false,
      },
    ],
  },

  {
    text: "Který měsíc je první v roce?",
    answers: [
      {
        text: "September",
        value: false,
      },
      {
        text: "Juli",
        value: false,
      },
      {
        text: "Januar",
        value: true,
      },
    ],
  },

  {
    text: "Kam mě odvezou, když se na lyžích zraním?",
    answers: [
      {
        text: "Krankenhaus",
        value: true,
      },
      {
        text: "Postamt",
        value: false,
      },
      {
        text: "Geschäft",
        value: false,
      },
    ],
  },

  {
    text: "Co si vezmu na záda, když vyrazím na túru?",
    answers: [
      {
        text: "Handtasche",
        value: false,
      },
      {
        text: "Rucksack",
        value: true,
      },
      {
        text: "Geldbeutel",
        value: false,
      },
    ],
  },
];
